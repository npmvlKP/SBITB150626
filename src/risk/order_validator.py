...
class OrderValidator:
    ...
    def validate_lot_size(self, symbol: str, quantity: int) -> OrderValidationResult:
        ...
        if "NIFTY" in symbol and symbol.endswith(("CE", "PE")):
            ...
            lot_size = self._position_settings.BANKNIFTY_LOT_SIZE
    ...
    def validate_tradingsymbol_format(self, tradingsymbol: str, exchange: str) -> OrderValidationResult:
        ...
        mcx_silver_pattern = r"^(SILVER)(\d{2})(\d{2})(\dP)\d+$"
            ...
            if not (
                re.match(mcx_gold_pattern, tradingsymbol) or re.match(mcx_silver_pattern, tradingsymbol)
            ):
                errors.append(f"Invalid tradingsymbol format: {tradingsymbol}. Expected patterns: {mcx_gold_pattern} or {mcx_silver_pattern}")
