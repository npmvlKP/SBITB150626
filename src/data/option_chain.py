...
class OptionMetricsComputer:
    ...
    def compute_single(
        ...
        # Validate inputs and compute time to maturity
        validation = self._validate_and_preprocess_inputs(
            spot, strike, expiry_date, as_of_date, option_ltp, option_type, risk_free_rate
        )
        if isinstance(validation, OptionMetrics):
            return validation
        t, flag = validation

        # Compute implied volatility
        iv = self._compute_implied_volatility(option_ltp, spot, strike, t, risk_free_rate, flag)
        if iv is None:  # IV computation failed
            return OptionMetrics(
                risk_free_rate=risk_free_rate,
                rfr_method=self._settings.RFR_METHOD,
                ttm_years=t,
                compute_error="iv_computation_failed",
            )

        # Compute Greeks
        greeks = self._compute_greeks(flag, spot, strike, t, risk_free_rate, iv)

        # Build and return result
        return self._build_result(iv, greeks, risk_free_rate, t)
    ...
    def _validate_and_preprocess_inputs(
        ...
        option_type: Literal["CE", "PE"],
        risk_free_rate: float,
    ) -> tuple[float, str] | OptionMetrics:
        """Validate inputs and preprocess for Greeks computation.

        Returns:
            Tuple of (time_to_maturity, flag) if validation passes,
            Optionmetrics error response if validation fails.
        """
    ...
    def _compute_implied_volatility(
        ...
        """Compute implied volatility with bounds checking.

        Returns:
            Implied volatility if successful, None if failed.
        """
        try:
            # Compute implied volatility
            iv = float(
    ...
    def _compute_greeks(
        ...
        """Compute Greeks using the provided inputs.

        Returns:
            Tuple of (delta, gamma, theta, vega) where any may be None on failure.
        """
        try:
            # Compute Greeks
    ...
    def _build_result(
        self,
        iv: float,
        greeks: tuple[float | None, float | None, float | None, float | None],
        risk_free_rate: float,
        t: float,
    ) -> OptionMetrics:
        """Build OptionMetrics result from computed values.

        Args:
            iv: Implied volatility
            greeks: Tuple of (delta, gamma, theta, vega)
            risk_free_rate: Risk-free rate
            t: Time to maturity in years

        Returns:
            OptionMetrics instance
        """
        d, g, th, v = greeks
        return OptionMetrics(
            iv=iv,
    ...
    async def compute_greeks_for_date(self, as_of_date: date, symbol: str) -> int:
        ...
        try:
            # Get risk-free rate
            rfr = await self._get_risk_free_rate(as_of_date)

            # Get database connection
            async with await self._get_database_connection() as conn:
                # Fetch option chain data
                options_rows = await self._fetch_option_chain(conn, symbol, as_of_date)
                if not options_rows:
                    return 0

                # Fetch spot price
                spot = await self._fetch_spot_price(conn, symbol, as_of_date)
                if spot is None:
                    return 0

                # Process in batches
                successful_count = await self._process_option_batches(
                    conn, options_rows, spot, as_of_date, symbol, rfr, batch_size
                )

        except Exception as e:
            logger.error(
                "greeks_computation_failed",
    ...
    async def _get_risk_free_rate(self, as_of_date: date) -> float:
    ...
    async def _fetch_spot_price(
        ...
        """Fetch spot price for the given symbol and date."""
        ...
        async with conn.cursor() as cur:
            ...
            spot_row = await cur.fetchone()
            return float(spot_row[0]) if spot_row else None
    ...
    async def _process_option_batches(
        ...
        """Process options in batches and return successful count."""
        successful_count = 0
        for i in range(0, len(options_rows), batch_size):
            ...
            if batch_metrics:
                await self._insert_greeks_batch(conn, batch_metrics)

            # Insert events to event log
            if batch_events:
                await self._insert_events_batch(conn, batch_events)

            # Update successful count
            successful_count += sum(1 for m in batch_metrics if m.compute_error is None)
    ...
    async def _process_batch(
        ...
        for row in batch:
            ...
            if metrics.compute_error is None:
                pass  # Success count will be calculated later
